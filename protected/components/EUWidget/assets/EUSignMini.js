...
function authForm_SignIn(DataToSignAddRandom) {
	var base = YiiBaseUrl();
	FillDataToSign();
...
//	Отримання комплекту сертифікатів основних центрів сертифікації та їх серверів
	jQuery.ajax({
	  type: 'GET',
	  url: base+'/index.php?r=sign/getcertificates',
	  dataType: 'json',
...
//	Збереження файлу сертифікату до директорії
		...
		jQuery.ajax({
		  type: 'POST',
		  url: base+'/index.php?r=sign/getstring',
...
//						Randstr = document.getElementById("SignRandstr").value;
						...
						if (typeof DataToSignAddRandom !== "undefined") {
							Signature = Signature + DataToSignAddRandom + Randstr;
						}

//alert("0:3 var signature");
						Signature = euSign.SignInternal("true", Signature);
						document.getElementsByName("AuthForm[Signature]")[0].value = Signature;
//alert("0:4 var signature");
						authForm_SetProxy();
//alert("0:5 var signature");
						euSign.ResetPrivateKey();
						euSign.Finalize();
