...
function authForm_SignIn() {
	var base = YiiBaseUrl();
	FillDataToSign();
//	Скрипт auth/getstring очікує послідовність символів GenerateAuthString як запит на генерування рядку автентифікації
...
//	Отримання комплекту сертифікатів основних центрів сертифікації та їх серверів
	jQuery.ajax({
	  type: 'GET',
	  url: base+'/index.php?r=auth/getcertificates',
	  dataType: 'json',
...
//	Збереження файлу сертифікату до директорії
		...
		jQuery.ajax({
		  type: 'POST',
		  url: base+'/index.php?r=auth/getstring',
...
//	Signature містить рядок, який був згенерований скриптом auth/getstring
//						Randstr = document.getElementById("SignRandstr").value;
//alert("0:2 before sign");
						var Signature = document.getElementsByName("AuthForm[Signature]")[0].value;
//alert("0:3 var signature");
						document.getElementsByName("AuthForm[Signature]")[0].value = euSign.SignInternal("true", Signature +";"+Randstr);
//alert("0:4 after sign");

			
//cypher

						var euCertificatesForm = EUCertificatesForm(frameHeight, frameWidth, 400, 600, true);
//alert("0:5 certificates form");
						euCertificatesForm.SelectCertificate("Сертифікати користувача", "CertificatesType.CertTypeEndUser", 
						function(ownersInfo){
							var issuers = euSign.CreateArrayList();
							var serials = euSign.CreateArrayList();
							var lastOwner;
					
							for (var i = 0; i < ownersInfo.length; i++) {
								alert(ownersInfo[i].GetSerial());
								var certex = euSign.GetCertificateInfoEx(ownersInfo[i].GetIssuer(), ownersInfo[i].GetSerial());
//								alert("string usage:"+certex.GetKeyUsage());
// 4 = ЕЦП, 16 = Розподіл ключів				alert("usage type:"+certex.GetKeyUsageType());
								
								issuers.add(euSign.GetCertificate(ownersInfo[i].GetIssuer(), ownersInfo[i].GetSerial()));
//								lastOwner = ownersInfo[i];
							}
					
							var envelopedString;
							try {
//								envelopedString = euSign.Envelop(lastOwner.GetIssuer(), lastOwner.GetSerial(), false, "123654");
//alert("0:6 before enveloptorecipients");

								envelopedString = euSign.EnvelopToRecipients(issuers, false, "123456");
								alert(envelopedString);
//	Одразу після підпису, зтерти зчитаний ключ з пам'яті криптобібліотеки
								euSign.ResetPrivateKey();
								euSign.Finalize();
//	Виконати передачу підпису на сервер з використанням форми auth-form
//								document.forms["auth-form"].submit();
								return true;
							} catch(e) {showException(e);
								alert(euSign.GetLastError()+"; код "+euSign.GetLastErrorCode());
							}
						},
