...
var ImportCertFormButtonID = "ImportCertFormButton";
...
//,,,
	var ImportCertFormButton = document.getElementById(ImportCertFormButtonID);
//	ImportCertFormButton.onclick = function(){importCertificate(); this.ShowForm("Сертифікати користувача", "CertificatesType.CertTypeEndUser"); return false};

	ImportCertFormButton.onclick = function(){
		try {
			var fileName = euSign.SelectFile(true, "*.cer");
			if(fileName == "")
				return;
			var certData = euSign.ReadFile(fileName);
			euSign.SaveCertificate(certData);
			
			thisForm._userSelected("CertificatesType.CertTypeEndUser");
			
//			var certInfo = euSign.ParseCertificate(certData);
//			this.CertTable.AddRow(certInfo.GetSubjCN(), certInfo.GetIssuerCN(), 				certInfo.GetSerial(), certInfo.GetSubject(), certInfo.GetIssuer());
		} catch(e) {
			showException(e);
		}
...
// added
	formContent += '<input type="button" value="Імпортувати сертифікат",\
						id=\"' + ImportCertFormButtonID + '\" style="width:285px;" class="button" />';
