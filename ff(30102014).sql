...
CREATE TABLE IF NOT EXISTS `cnap_portal`.`ff_ref_multiguide` (
  ...
  `owner_field` INT(11) NULL DEFAULT NULL,
...
BEGIN
	...
	if (TYPEREF=14) then
		INSERT INTO `ff_field` (`formid`,`name`,`type`,`description`,`order`,`protected`)
		SELECT src1.`formid`, concat(src1.`name`,'_fileedsname'), 15, concat(src1.`description`,' - Имя файла'), 0,1
		FROM `ff_field` src1 WHERE (src1.`id`=ID);
		INSERT INTO `ff_field` 
			(`formid`,`name`,`type`,`description`,`order`,`protected`)
		SELECT src2.`owner`, src1.`name`, src1.`type`, src1.`description`, src1.`order`,1
...
DECLARE cur1 CURSOR FOR	select sq from (
	select ff_cn, ff_tp, is_cn, is_tp,
		case 
			when (ff_cn = is_cn) and (ff_tp <> is_tp) then CONCAT(' MODIFY `', ff_cn, '` ', ff_tp,' DEFAULT ', if((ff_df is null) or (lower(ff_df)='null'), 'NULL', ff_df))
			when (ff_cn is null) then CONCAT(' DROP `', is_cn,'`')
			when (is_cn is null) then CONCAT(' ADD `', ff_cn, '` ', ff_tp,' DEFAULT ', if(ff_df is null, 'NULL', ff_df))
			else null
-- CONCAT(' MODIFY `', ff_cn, '` ', ff_tp,' DEFAULT ', if((ff_df is null) or (lower(ff_df)='null'), 'NULL', ff_df))
		end sq
	from 
	((SELECT 
...
BEGIN
	...
	if (VCNT>0) then
		RETURN IDREGISTRY1;
	end if;	select t.`parent`, cnt INTO VCNT, VCNT2 from
