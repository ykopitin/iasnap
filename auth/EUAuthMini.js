...
//	Отримання комплекту сертифікатів основних центрів сертифікації та їх серверів
	jQuery.ajax({
	  type: 'GET',
	  url: '/index.php?r=auth/getcertificates',
	  dataType: 'json',
	  success: function(ret_data){
		...
		jQuery.ajax({
		  type: 'POST',
		  url: '/index.php?r=auth/getstring',
...
//	data.randstr містить рядок автентифікації
			var Signature = data.randstr;
//			document.getElementsByName("Signature")[0].value=data.randstr;
...
//								envelopedString = euSign.Envelop(lastOwner.GetIssuer(), lastOwner.GetSerial(), false, "123654");
								envelopedString = euSign.EnvelopToRecipients(issuers, false, "123654");
