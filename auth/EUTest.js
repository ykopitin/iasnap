...
//	Скрипт certcnap.php очікує послідовність символів GetCertificate як запит на отримання сертифікату ЦНАП
		$send = "GetCertificate";
...
//	Функція перетворення Base64.Encode міститься у файлі base64ie.js, який підключається у шаблоні auth.xsl
		...
		jQuery.ajax({
		  type: 'POST',
		  url: '/auth/cnapcert.php',
		  data: {ask: $send},
		  dataType: 'json',

		  success: function(data){
			euSign.WriteFile(settings.GetPath() + "\\cnapcert.crt", data.cert);
		  },
...
//	проходження процедури підписання даних та відправки підписаних даних за допомогою форми Authorize
...
//	Спроба зчитування збережених параметрів файлового сховища
				try {
					...
					settings = euSign.GetFileStoreSettings();
					document.getElementById('OwnCertPath').value = settings.GetPath();
//	Todo: Додати перевірку на можливість запису нових файлів, якщо немає сертифікатів користувачів
				} catch(e) {
					try {
						regFormUtils_DefaultFileSettings();
						var settings1 = euSign.CreateFileStoreSettings();
						...
						alert("Помилка створення параметрів файлового сховища. Оберіть іншу директорію для збереження сертифікатів. "+euSign.GetLastError());
						euSign.Finalize();
						return;
					}
				}
...
//	Signature містить рядок, який був згенерований скриптом genstr.php
						document.getElementsByName("Signature")[0].value = euSign.SignInternal("true", Signature);
//	Одразу після підпису, зтерти зчитаний ключ з пам'яті криптобібліотеки
						euSign.ResetPrivateKey();
						euSign.Finalize();
//	Виконати передачу підпису на сервер з використанням форми Authorize
...
function showStep(goStepNumber) {
	...
	if (curStepNumber == 1) {	// Якщо перехід виконується з первшой вкладки (вхід), то ініціалізувати Java-модуль для інших вкладок
		try {
			var euisinit = euSign.IsInitialized();
			if (euisinit == false) {
				euSign.SetCharset("UTF-16LE");
